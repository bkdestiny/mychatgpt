...
import com.mychatgpt.dto.ChatMessage;
import com.mychatgpt.dto.ChatRequestParameter;
import com.mychatgpt.dto.ChatResponseParameter;
import com.mychatgpt.dto.Choice;
...
public class ChatModel {
     ...
     * @param chatRequestParameter 请求参数
     * @param question                问题
     * @return 返回chatGpt给出的答案
     */
    public String getAnswer(Consumer<String> resConsumer, ChatRequestParameter chatRequestParameter, String question) {
        asyncClient.start();
        // 创建一个post请求
        AsyncRequestBuilder asyncRequest = AsyncRequestBuilder.post(url);

        // 设置请求参数
        chatRequestParameter.addMessages(new ChatMessage ("user", question));

        // 请求的参数转换为字符串
        String valueAsString = null;
        try {
            valueAsString = objectMapper.writeValueAsString(chatRequestParameter);
        ...
        asyncClient.execute(asyncRequest.build(), consumer, new FutureCallback<HttpResponse>() {
            ...
            public void completed(HttpResponse response) {
                latch.countDown();
                chatRequestParameter.addMessages(new ChatMessage ("assistant", sb.toString()));
